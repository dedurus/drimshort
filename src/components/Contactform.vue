<template>
  ...
  <ValidationObserver ref="observer" v-slot="{ handleSubmit, invalid }">
    <b-alert :show="form_state.success" variant="dark" @dismissed="alerts.dismissCountDown = 0"
      @dismiss-count-down="countDownChanged">
      Message sent. Thank you!
    </b-alert>

    <b-alert :show="form_state.error" variant="dark">
      An error occured. Please try again. <span v-if="form_state.error_code">(Err. code:
        {{ form_state.error_code }})</span>
    </b-alert>

    <b-form @submit.prevent="handleSubmit(onSubmit)" @reset="resetForm" class="mb-2">
      <div class="row">
        <div class="col-md-6 mx-auto">

          <BTextInputWithValidation rules="required" name="Name" vid="name" type="text" label="Name" v-model="form.name"
            placeholder="Let us know your name." />


          <BTextInputWithValidation rules="required|email" type="email" label="Email address:" name="Email"
            v-model="form.email" placeholder="Your email address" />
        ...
        <div class="col-md-6 mx-auto">
          <MessageInput rules="required" name="Message" vid="message" type="text" label="Message" v-model="form.message"
